function LoadCommandPanel()
	...
	if table.HasValue( ELITE.AccessToCPCmds, LocalPlayer():Team() ) then
		table.insert( Categories, { Name = "POLICE", Table = CPCMD_BUTTONS } )
	end
	if table.HasValue( ELITE.AccessToMayorCmds, LocalPlayer():Team() ) then
		table.insert( Categories, { Name = "MAIRE", Table = MAYORCMD_BUTTONS } )
	end
	table.insert( Categories, { Name = "AUTRE", Table = OTHERCMD_BUTTONS } )
