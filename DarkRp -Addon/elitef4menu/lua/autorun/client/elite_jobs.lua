	...
	for k, v in pairs( RPExtraTeams ) do
		...
		if show then
			...
			jobFrame.Paint = function( self, w, h )
				...
				draw.SimpleText( "SALARY: $"..string.upper(v.salary), "RP_SubFontThick", 56, 30, Color( 190, 190, 190 ) )
			end
				...
				if type(v.model) == "table" and table.Count( v.model ) > 1 then
					...
					if v.vote then
						if ((v.admin == 0 and LocalPlayer():IsAdmin()) or (v.admin == 1 and LocalPlayer():IsSuperAdmin())) then
								...
								menu:AddOption("Do not vote", function() RunCmd("/"..v.command) BG_PANEL:Remove() end)
								menu:Open()
						else
								RunCmd("/vote"..v.command)
								BG_PANEL:Remove()
...
function LoadCmdPanel()
	...
	JOB_SIDE.Paint = function( self, w, h )
		...
		draw.SimpleText( "JOBS", "RP_SubFontThick", 24, 20, Color( 190, 190, 190 ) )
	...
	if ELITE.VIPJobCheckbox then
		...
		VIP_CHECK:SetText( "Show VIP Jobs" )
